...
export class EngineComponent {
  ...
  constructor(private fb:FormBuilder,private viewPortScroller:ViewportScroller,private runService:RunService){
    ...
    this.val_ini=this.fb.group({
      ...
      plazo_p:[""],
      cok:["",[Validators.required],[Validators.required,Validators.min(0)]]
    }
    )
  }
  pasarDatos(){
  ...
  validarAparicionPlazo(e:any){
    if(e=="Parcial" || e=="Total"){   
      this.hallarPeriodosSimu();
      this.aparcion_tiem_plazo=true
      this.val_ini.get("plazo_p")?.addValidators([Validators.required,Validators.max(this.numPeriodosSimu),Validators.min(1)]);
    }
    else{
      this.aparcion_tiem_plazo=false
      this.val_ini.get("plazo_p")?.removeValidators([Validators.required,Validators.max(this.numPeriodosSimu),Validators.min(1)]);
    }
  }
  hallarPeriodosSimu(){
    let anios_meses=this.val_ini.get("tie_p")?.value * 12;
    if(this.val_ini.get("frec_p")?.value=='Mensual'){
      ...
      this.frecPagoSimu=12;
    }

    this.numPeriodosSimu=anios_meses/this.frecPagoSimu;
  }
  hallarPeriodos(){
