...
public class FinancialRunController {
    ...
    public ResponseEntity<FinancialRun> createFinancialRun(@RequestBody FinancialRun run,@PathVariable("id") Long id) throws ParseException {
        ...
        System.out.println(run.getCok());
        System.out.println(run.getVan());
        System.out.println(run.getTir());
        FinancialRun newRun = financialRunRepository.save(new FinancialRun(run.getTypeMoney(), run.getPriceProperty() , run.getFirstFee(),run.getAmountFinance(),run.getNameRate(),run.getTimeRate(),run.getRate(),run.getFrequencyPay(),run.getConvertRate(),run.getNameConvertRate(),run.getNumberYear(),run.getNumberPeriods(),run.getGracePeriod(),run.getNumberGracePeriod(),run.getCok(),run.getVan(), run.getTir(),customerFound));
